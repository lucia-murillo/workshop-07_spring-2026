...
H~0~: Elevation (m) does not predict temperature (°C) in the Sonadora mountains.

H~A~: Elevation (m) predicts temperature (°C) in the Sonadora mountains.
...
ggplot(data = sonadora_sum, 
  ...
  labs(x = "Elevation (m)",
       ...
       title = "Mean temperature decreases with elevation") +
...
temperature_model <- lm(
  ...
  data = sonadora_sum # data from summary 
  )
