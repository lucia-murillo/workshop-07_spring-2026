...
H~A~: **pH has an effect on abalone growth.**
...
# creating new object from abalone dataframe 
...
ggplot(data = abalone_clean, # using abalone_clean data 
       aes(x = mean_ph, # x-axis: mean pH, y-axis: shell surface area change  
           y = change_in_area)) +
  # first layer: geom_point to represent individual observation 
...
**The missing value comes from a pair of tanks in which the pH sensor malfunctioned, so there is no pH measurement for those tanks. **  
...
abalone_model <- lm(
  ...
  data = abalone_clean      # data from: clean abalone data 
)
```
...
(not worrying about `group` for now) 
...
**At pH = 8, the model predicts abalone shell growth of is 5.36 mm per day [95% CI = 5.08, 5.64].**
